...
turtles-own [
  way-home            ;; memorizar direccion pa donde esta casita
]
...
to setup
  clear-all            ;; Se limpia el mundo
  set-default-shape turtles "bug"  ;;Se setea la figura por defecto de las turtles
  create-turtles population        ;;Se crea la poblacion de turtles segun el valir
                                   ;;de poblacion dado por el slider
  [ set size 2         ;; tamaño de las tortugas
    set color red  ]   ;; color de las tortugas
  setup-patches        ;; Se configuran los patches
  reset-ticks          ;; Se resetean los ticks del mundo
end
...
to go
  ask turtles                ;; Configuracion de las tortugas
  [ if who >= ticks [ stop ] ;; delay initial departure
    ifelse color = red       ;; en caso de estar en estado sin comida (rojo)
    [ look-for-food  ]       ;; funcion para buscar por comida en un angulo
    [ return-to-nest ]       ;; volver al nido siguiendo el nest chemical
    wiggle                   ;; rotacion para buscar quimicos
    fd 1 ]                   ;; moverse para adelante
  diffuse chemical (diffusion-rate / 100)    ;; variable de difucion
  ask patches                ;; Configurar patches
  [ set chemical chemical * (100 - evaporation-rate) / 100  ;; evaporacionde quimicos
    recolor-patch ]                                         ;; poner color segun el nivel de quimicos
  tick
end
...
to return-to-nest  ;; turtle procedure
  ...
  [ set chemical chemical + 60  ;; drop some chemical
    uphill-nest-scent ]
    ;; go-back-home ]         ;; de vuelta a casita
end

to go-back-home ;; turtle procedure
  facexy 0 0  ;; mirar hacia 0,0
  fd 1        ;; ir en esa direccion
  set chemical chemical + 20 ;;deja el quimico a la vuelta
end
...
;; escaneo direccional para detectar el olor mas grande del quimico,
to uphill-chemical  ;; turtle procedure
  let scent-ahead chemical-scent-at-angle   0   ;; Verificacion de chemical scent en parche en frente
  ...
  [ ifelse scent-right > scent-left ;; Rotacion en direccion respectia dependiendo de cual sea el mayor
    [ rt 45 ]
    [ lt 45 ] ]
end
...
;;rotar entre 40 y -40 grados antes de moverse
